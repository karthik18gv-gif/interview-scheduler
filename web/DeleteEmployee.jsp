<%-- 
    ...
    Created on : Jan 30, 2012, 6:12:53 PM
    Author     : PHP
--%>
...
function valid()
            {
                if(document.delempfrm.delemp.value==""||document.delempfrm.delemp.value==null)
                    {
                        alert("Please Select Employee UserName To Delete")
                        return false
                    }
            }
</script>
...
<div id="templatemo_container">
    ...
    <div id="templatemo_content_wrapper">
        ...
        if (sess.getAttribute("admin").equals("123")) {
            ...
            %>
<h3 align="center">Delete Employee From the HR List</h3>
  <form name="delempfrm" style="font-size: 14px;font-family: fantacy;color: white" action="deleteemp" onsubmit="return valid()">
            <%
            st=con.createStatement();
            rs=st.executeQuery("select empusername from employee");
            %>

        <br/><br/>Select Employee username from the below to delete the Employee from the List
        <br/><br/><select name="delemp">
            <option value="">select</option>
            <%
            while(rs.next())
                {
            %>
            <option><%=rs.getString("empusername")%></option>
            <%
            }
            %>
        </select>
        <br/><br/><br/>
        <input type="submit" value="Delete Employee from HR List">
